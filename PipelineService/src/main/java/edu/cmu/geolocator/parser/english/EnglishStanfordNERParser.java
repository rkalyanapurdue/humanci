...
public class EnglishStanfordNERParser implements NERTagger {
  ...
  private static EnglishStanfordNERParser englishparser;

  public static EnglishStanfordNERParser getInstance() throws Exception {
      if (englishparser == null) {
        try {
          englishparser = new EnglishStanfordNERParser();
        } catch (Exception e) {
  ...
  public EnglishStanfordNERParser() {
    String classifierPath = "/home/ec2-user/geo-locator/en/english.all.3class.distsim.crf.ser.gz";
  ...
  public List<LocEntityAnnotation> parse(Tweet tweet) {
    ArrayList<LocEntityAnnotation> le = new ArrayList<LocEntityAnnotation>();
    ...
    while (matcher.find()) {
      if (entities.contains(matcher.group(1))) {
         ...
         */
        if (add) {
          le.add(new LocEntityAnnotation(matcher.start(0), matcher.end(0), "LOCATION",
                  new Token[] { new Token(elem, tweet.getId(), matcher.start(0)) }));
        }
      }
    }
    tweet.setToponyms(le);
    return le;
  }
